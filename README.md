...
このリポジトリは、**「ComfyUI API」を活用した自律思考型AIシステム**のプロトタイプです。
ComfyUIをPythonスクリプトからAPI経由で直接制御することで、手動操作では不可能な「LLMと画像生成の完全自動ループ」を実現しています。

### 技術的な特徴
1.  **ComfyUI API制御**: JSON形式のワークフローをPythonで動的に書き換え、WebSocketでリアルタイムに生成指示を送ります。
2.  **文脈理解**: ユーザーの短いキーワードから物語を膨らませる（脚本家AI）。
3.  **品質管理**: 生成された画像をAIが自分の目でチェックし、崩れていれば修正指示を出す（検品AI）。

**【お仕事の依頼について】**
ComfyUIのワークフロー構築から、Python APIを用いた自動化ツールの開発まで承っております。
APIコストをかけないローカル環境構築や、特定の画風に特化させたパイプラインの作成など、お気軽にご相談ください。
